...
for i = 1:4 %do the problem for each starting point
    ...
    fval_v1 = pol(gon(1,1),gon(1,2)); %pollution at point 1
    fval_v2 = pol(gon(2,1),gon(2,2)); %pollution at point 2
    fval_v3 = pol(gon(3,1),gon(3,2)); %pollution at point 3
    ...
    while error > tol 
        ...
        %B0 = B; %for error purposes below
        %fvalB0 = pol(B0(1),B0(2));
        ...
        if fvalR < fvalG %what the book calls case i - reflect or extend
            ...
            C1 = (W + M)./2;
            C2 = (M + R)./2;
            fvalC1 = pol(C1(1),C1(2));
            fvalC2 = pol(C2(1),C2(2));
            if fvalC1 < fvalC2
                C = C1;
                fvalC = fvalC1;
            else
            ...
            if fvalC < fvalW
                ...
                S = (B + W)./2;
                fvalS = pol(S(1),S(2)); %is this even necessary?
                W = S;
                G = M;
            end
        end
    ...
    plot(B(1),B(2),'blackx','Linewidth',2)
    lstring = sprintf('Maximum pollutant in channel: %4.5f',fvalB);
